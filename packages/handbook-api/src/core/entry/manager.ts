 ...
 */
export function createHandbookEntryDataManager(
  routePrefix = ''
): EntryDataManagerConstructor {
  return class HandbookEntryDataManager extends EntryDataManager {
    /**
     * @override
     */
        ...
        for (let i = 0, p = ''; i < location.pieces.length; ++i) {
          ...
          if (child == null || child.title !== location.pieces[i]) {
            if (i + 1 < location.pieces.length) {
              ...
              child = {
                title,
                pathname: resolveUrlPath(routePrefix, p, title),
                source: resolveUrlPath(
                  this.urlRoot, HandbookSourceType.POST, location.uuid + '.json'),
              }
            }
            children.push(child)
          }
