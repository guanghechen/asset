...
describe('AssetDataProvider', function () {
  jest.spyOn(global.console, 'info').mockImplementation(function (...args: string[]): any {
  ...
  for (const kase of cases) {
    ...
    describe(kase.title, function () {
      ...
      const fsWriteJsonSyncMock = jest
        .spyOn(fs, 'writeJSON')
        .mockImplementation(function (absolutePath: string | any, data: any): any {
          const snapshotName = path.relative(siteConfig.workspace, absolutePath)
          // eslint-disable-next-line jest/no-standalone-expect
          expect(desensitize(data, assetDataReplacer)).toMatchSnapshot(desensitize(snapshotName))
        })
        ...
        describe(subSiteName, function () {
          ...
          const fakeProcessor: AssetProcessor = {
            types: () => [
              {
                type: 'image',
                assetDataRoot: subSiteConfig.source.image.dataRoot,
              },
            ],
            ...
            process: function* (
              ...
              const categories: CategoryDataItem[][] = (data.categories || []).map(
                (categoryPath: string[]) => categoryPath.map(c => categoryDataManager.normalize(c)),
              )
          ...
          const getCurrentData = (provider: AssetDataProvider<any>): unknown[] => {
            const assets = Object.keys(subSiteConfig.source).reduce((acc, assetType) => {
          ...
          const baseTest = function (provider: AssetDataProvider<any>): void {
            ...
            expect(categories.map(c => provider.categoryService.fetchCategory(c.uuid))).toEqual(
              categories,
            )
            ...
            for (const assetType of Object.keys(subSiteConfig.source)) {
              ...
              expect(provider.assetService.fetchAssets(assetType, 0, assets.length)).toEqual(assets)
              expect(provider.assetService.fetchAssets(assetType, 0, assets.length - 1)).toEqual(
                assets.slice(0, -1),
              )
          ...
          test('build', async function () {
            const provider = new AssetDataProvider({
              subSiteConfig,
          ...
          test('start', async function () {
            ...
            const testImageFilepath = path.resolve(
              subSiteConfig.source.image.sourceRoot,
              'provider.test.image.json',
            )
