    ...
    describe(kase.title, function () {
      ...
      for (const subSiteName of subSiteNames) {
        ...
        describe(subSiteName, function () {
          ...
          const fakeEntityManagerMap: Record<
            string,
            AssetEntityManager<AssetEntity>
          > = {}
          ...
          const getCurrentData = (
            provider: AssetDataProvider<any>,
          ): unknown[] => {
            const assets = Object.keys(subSiteConfig.source).reduce(
              (acc, assetType) => {
          ...
          const baseTest = function (provider: AssetDataProvider<any>): void {
            ...
            expect(
              provider.categoryService.fetchCategory('non-exist uuid'),
            ).toBeNull()
            ...
            for (const assetType of Object.keys(subSiteConfig.source)) {
              ...
              expect(
                provider.assetService.fetchAsset('non-exist uuid'),
              ).toBeNull()
              ...
              expect(
                provider.assetService.fetchAssets(
                  assetType,
                  0,
                  assets.length - 1,
                ),
          ...
          test('start', async function () {
            ...
            expect(
              provider.categoryService
                .fetchCategories()
                .filter(category =>
                  testImageData.categories.flat().includes(category.title),
                )
