import { cover, coverString, isNonBlankString, isNotEmptyArray } from '@guanghechen/option-helper'
...
export const resolveSubSiteSourceItem: SubSiteSourceItemResolver<SubSiteSourceItem> = (
  ...
  const sourceRoot: string = resolveLocalPath(
    subSiteSourceRoot,
    coverString(defaultSourceItem.sourceRoot, rawSourceItem.sourceRoot, isNonBlankString),
  )

  // resolve dataRoot (absolute filepath)
  const dataRoot: string = resolveLocalPath(
    subSiteDataRoot,
    coverString(defaultSourceItem.dataRoot, rawSourceItem.dataRoot, isNonBlankString),
  )
  ...
  const encoding: BufferEncoding | undefined = cover<BufferEncoding | undefined>(
    defaultSourceItem.encoding,
    rawSourceItem.encoding,
    isNonBlankString,
  )
